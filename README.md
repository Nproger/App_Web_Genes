...
- 🎯 Entry points to execute your program using `python -m <app_web_genes>` or `$ app_web_genes` with basic CLI argument parsing.
...
---
# app_web_genes Flask Application

Awesome app_web_genes created by Nproger
...
```bash
git clone https://github.com/Nproger/App_Web_Genes app_web_genes
cd app_web_genes
make install
```
...
```bash
app_web_genes create-db   # run once
app_web_genes populate-db  # run once (optional)
app_web_genes add-user -u admin -p 1234  # ads a user
app_web_genes run
```
